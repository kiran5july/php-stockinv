...
private $hideheader;//--KM--
private $angle;//--KM--
private $pdf;//--KM--
//private $pdf->$balancetext;//--KM--
private $arraygrouphead = array();//--KM--
private $arraygroupfoot = array();//--KM--
...
//variable not yet implemented
    ...
    public function background() {
        foreach ($this->arraybackground as $out) {
            switch($out){  //["hidden_type"]) { --KM--COMMENTED
                case "field":
    ...
    public function title() {
        ...
        foreach ($this->arraytitle as $out) {
            ...
            switch(isset($out["hidden_type"]) ? $out["hidden_type"] : "") { //--KM--
                case "field":
    ...
    public function pageFooter() {
        ...
        if(isset($this->arraypageFooter)) {
            foreach ($this->arraypageFooter as $out) {
                switch((isset($out["hidden_type"])?$out["hidden_type"]:"")) {
                    case "field":
	...
	//	$this->pdf->MultiCell(200,10,"????",1);
		...
		//	$this->currentband='detail';
   			$n++;
				...
				if($n>1)
					$checkpoint=isset($this->maxpagey['page_'.($this->pdf->getNumPages()-1)])?$this->maxpagey['page_'.($this->pdf->getNumPages()-1)]:"";
      //                   echo $checkpoint."<br/>";
...
*/
    public function showGroupHeader($y) {
        ...
        $bandheight=(isset($this->arraygrouphead['height'])?$this->arraygrouphead['height']:"");//--KM-- removed [0] to make 1D array
    ...
    public function showGroupFooter($y) {
        ...
        $bandheight=(isset($this->arraygroupfoot['height'])?$this->arraygroupfoot['height']:"");//--KM-- removed [0] to make 1D array
  ...
  //print_r($arraydata);echo "<br/>";
    ...
    $sRotationKM = isset($arraydata["rotation"]) ? $arraydata["rotation"] : ""; //--KM--
    $this->Rotate($sRotationKM);
    ...
    public function checkoverflow($arraydata,$txt="",$maxheight=0) {
        ...
        if($this->print_expression_result==true) {
            ...
            elseif($arraydata["poverflow"]=="false"&&$arraydata["soverflow"]=="true") {
				...
				$x=$this->pdf->GetX();
		        $this->pdf->MultiCell($arraydata["width"], $arraydata["height"], $this->formatText($txt, (isset($arraydata["pattern"])?$arraydata["pattern"]:""))
							, $arraydata["border"] 
...
//                $arrdata[$num]=&$this->arrayVariable[substr($out,3,-1)]["ans"];
    ...
    public function print_expression($data) {
        $expression=(isset($data["printWhenExpression"])?$data["printWhenExpression"]:"");
